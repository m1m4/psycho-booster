...
export const useResponsiveFontSize = (text: string, baseSize = 'text-lg') => {
    return useMemo(() => {
        ...
        if (len < 60) return 'text-2xl leading-relaxed'; 
        if (len < 150) return 'text-xl leading-relaxed';
        if (len > 600) return 'text-sm leading-relaxed'; // Very long texts
        if (len > 400) return 'text-base leading-relaxed'; // Long texts
        return 'text-lg leading-relaxed'; // Standard
    }, [text, baseSize]);
};
...
// PreviewImage moved to @/components/ui/PreviewImage
